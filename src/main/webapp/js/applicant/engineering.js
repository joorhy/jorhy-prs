...
function editEngineering(){
    var row = $('#dgEngineering').datagrid('getSelected');
    if (row){
        ...
        dlg_type = 'edit';
        edit_index = $('#dgEngineering').datagrid('getRowIndex', row);
    }
}
...
function removeEngineering(){
    ...
    if (row){
        $.messager.confirm('操作提示','是否确认删除此项目?',function(r){
            if (r){
                var index =  $('#dgEngineering').datagrid('getRowIndex', row);
                $('#dgEngineering').datagrid('deleteRow', index);
            }
        });
    }
}